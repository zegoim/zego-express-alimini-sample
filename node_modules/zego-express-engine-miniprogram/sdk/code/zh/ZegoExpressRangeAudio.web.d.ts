import { ZegoRangeAudioMode, ZegoRangeAudioEvent } from "./ZegoExpressEntity.web";
...
export declare class ZegoExpressRangeAudio {
    ...
    private zegoAudioListener;
    private constructor();
    /**
     * @param engine -
     */
    static getInstance(engine: any, logger: any): ZegoExpressRangeAudio;
    /**
     * 注册回调事件。
     *
     ...
     *
     * @return 标识是否接口是否调用成功。失败原因可能是浏览器不支持或没有使用特殊编包。
     */
     ...
     *
     * Note: 详情描述：通过传入扬声器设备的 deviceID 来指定范围语音播放音频使用的设备，deviceID 可以通过 ZegoExpressEngine.getMicrophones 接口获取。
     *
     ...
     * Note: 调用时机：调用接口 createRangeAudioInstance 创建实例后。
     *
     * Note: 相关接口：获取麦克风设备列表接口 getSpeakers。
     *
